...
%token IF     ELSE        RETURN   BREAK   NEW        FI DO OD
%token PRINT  READ_INTEGER         READ_LINE
%token LITERAL
%token IDENTIFIER	  AND    OR    STATIC  INSTANCEOF NUMINSTANCES
%token GUARD_SEPERATOR
%token INC    DEC
...
Expr            :	LValue
                ...
                |   Expr '?' Expr ':' Expr
                    {
                        $$.expr = new Tree.Ternary(Tree.TERNARY_COND, $1.expr, $3.expr, $5.expr, $2.loc);
                    }
                |	Expr '+' Expr
                	{
...
GuardStmt :     IF GuardClauses FI
                    {
                        $$.stmt = new Tree.Guard(Tree.Guard.Type.Condition, $2.guard_clauses, $1.loc);
                    }
                |
                DO GuardClauses OD
                    {
                        $$.stmt = new Tree.Guard(Tree.Guard.Type.Loop, $2.guard_clauses, $1.loc);
                    }
                ;

GuardClauses : GuardClauses GUARD_SEPERATOR Expr ':' Stmt
                    {
                        $$.guard_clauses.add(new Tree.GuardClause($3.expr, $5.stmt, $2.loc));
                    }
                |   Expr ':' Stmt
                    {
                        $$.guard_clauses = new ArrayList<Tree.GuardClause>();
                    	$$.guard_clauses.add(new Tree.GuardClause($1.expr, $3.stmt, $2.loc));
                    }
                ;
